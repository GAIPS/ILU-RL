    ...
    def run(
        ...
        for step in tqdm(range(num_steps)):
            ...
            if communicate and step > 10:
                ...
                resids.append(resid(data, np.nanmean(step_speeds)))
                if step == 20:
                    resid_plot(resids, step,
                             save_path='data/emissions/consensus/grid_20210227-1901001614452460.5683067/')
                print(step, veh_speeds[-1], num_com, len(resids))
                break
            else:
...
def resid(data, target):
    if isinstance(data, list):
        X = np.stack(data)
    T = np.matlib.repmat(target, *X.shape)
    err = np.power((X - T), 2)
    return err


def resid_plot(residuals, time, save_path=None):
    N = len(residuals)
    X = np.nanmean(np.stack(residuals), axis=0)
    ...
    ax.set_yscale('log')
    ax.plot(X)
    plt.legend(('error 1', 'error 2', 'error 3'))
    plt.show()
    if not save_path is None:
        plt.savefig(f'{save_path}/err_{time}.png', bbox_inches='tight', pad_inches=0)
    plt.close()

def mse_plot(data, target, time, save_path=None):
    MAX_STEPS = 25
    num_steps = np.arange(0, MAX_STEPS + 1, 1)
    ...
    plt.show()
    if not save_path is None:
        plt.savefig(f'{save_path}/mse_{time}.png', bbox_inches='tight', pad_inches=0)
    plt.close()
    return err
