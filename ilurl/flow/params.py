...
class VehicleParams:
    ...
    def __init__(self):
        ...
        #: dict (str, int) : contains the min_gap attribute of each type of
        #: vehicle
        self.min_gap = dict()
    ...
    def add(self,
        ...
        car_following_params : ilurl.ilurl.flow.params.SumoCarFollowingParams
            Params object specifying attributes for Sumo car following model.
        lane_change_params : ilurl.ilurl.flow.params.SumoLaneChangeParams
        ...
        # instantiation.
        self.min_gap[veh_id] = type_params["min_gap"]
...
class SumoCarFollowingParams:
    ...
    max_speed : float
        see max_speed Note
    speed_factor : float
        see speed_factor Note
    speed_dev : float
        see speedDev in Note
    impatience : float
        see Note
    car_follow_model : str
        see car_follow_model in Note
    kwargs : dict
    ...
    def __init__(
        ...
        # check for deprecations (min_gap)
        if "min_gap" in kwargs: min_gap = kwargs["min_gap"]

        # check for deprecations (max_speed)
        if "max_speed" in kwargs: max_speed = kwargs["max_speed"]

        # check for deprecations (speed_factor)
        if "speed_factor" in kwargs: speed_factor = kwargs["speed_factor"]

        # check for deprecations (speedDev)
        if "speedDev" in kwargs: speed_dev = kwargs["speedDev"]

        # check for deprecations (car_follow_model)
        if "car_follow_model" in kwargs: car_follow_model = kwargs["car_follow_model"]
...
class SumoLaneChangeParams:
    ...
    def __init__(self,
        ...
        # check for deprecations (lc_strategic)
        if "lc_strategic" in kwargs: lc_strategic = kwargs["lc_strategic"]

        # check for deprecations (lc_cooperative)
        if "lc_cooperative" in kwargs: lc_cooperative = kwargs["lc_cooperative"]

        # check for deprecations (lc_speed_gain)
        if "lc_speed_gain" in kwargs: lc_speed_gain = kwargs["lc_speed_gain"]

        # check for deprecations (lc_keep_right)
        if "lc_keep_right" in kwargs: lc_keep_right = kwargs["lc_keep_right"]

        # check for deprecations (lc_look_ahead_left)
        if "lc_look_ahead_left" in kwargs: lc_look_ahead_left = kwargs["lc_look_ahead_left"]

        # check for deprecations (lc_speed_gainRight)
        if "lc_speed_gainRight" in kwargs: lc_speed_gain_right = kwargs["lc_speed_gainRight"]

        # check for deprecations (lc_sublane)
        if "lc_sublane" in kwargs: lc_sublane = kwargs["lc_sublane"]

        # check for deprecations (lc_pushy)
        if "lc_pushy" in kwargs: lc_pushy = kwargs["lc_pushy"]

        # check for deprecations (lc_pushy_gap)
        if "lc_pushy_gap" in kwargs: lc_pushy_gap = kwargs["lc_pushy_gap"]

        # check for deprecations (lc_assertive)
        if "lc_assertive" in kwargs: lc_assertive = kwargs["lc_assertive"]

        # check for deprecations (lcAccelLat)
        if "lc_accel_lat" in kwargs: lc_accel_lat = kwargs["lc_accel_lat"]
        ...
        if model == "LC2013":
            self.controller_params = {
                ...
                "lc_strategic": str(lc_strategic),
                "lc_cooperative": str(lc_cooperative),
                "lc_speed_gain": str(lc_speed_gain),
                "lc_keep_right": str(lc_keep_right),
                # "lc_look_ahead_left": str(lc_look_ahead_left),
                # "lc_speed_gainRight": str(lc_speed_gainRight)
            }
        elif model == "SL2015":
            self.controller_params = {
                "laneChangeModel": model,
                ...
                "lc_keep_right": str(lc_keep_right),
                "lc_look_ahead_left": str(lc_look_ahead_left),
                "lc_speed_gainRight": str(lc_speed_gain_right),
                "lc_sublane": str(lc_sublane),
                "lc_pushy": str(lc_pushy),
                "lc_pushy_gap": str(lc_pushy_gap),
                "lc_assertive": str(lc_assertive),
                "lc_accel_lat": str(lc_accel_lat)
            }
