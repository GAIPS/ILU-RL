...
from models.evaluate import main as roll
...
def get_arguments():
    parser = argparse.ArgumentParser(
        description="""
            This scripts runs recursively every experiment on path.
            It must receive an experiment path.
        """
    )
    ...
    parser.add_argument('--test', '-t', dest='test', type=str2bool,
                        default=False, nargs='?',
                        help='If true only last checkpoints will be used.')
...
def rollout_batch(test=False, experiment_dir=None):
    ...
    # Read train.py arguments from rollouts.config file.
    ...
    if test:
        ...
        experiment_time = test_config.get('test_args', 'experiment-time')
        emission = test_config.get('test_args', 'sumo-emission')
        seed_delta = int(test_config.get('test_args', 'seed_delta'))

        # Overwrite defaults.
        rollouts_config.set('rollouts_args', 'experiment-time', experiment_time)
